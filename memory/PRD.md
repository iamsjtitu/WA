...
| 15 | testing_agent verdict: 100% (5/5 pytest + 4/4 curl + 3/3 UI) | **DB-vs-Node desync fix (intermittent "session disconnected" 400)** — the daily false-positive that another Emergent agent (rice-mill customer) diagnosed and reported. Root cause: Baileys silently reconnects but the DB `status` flag lagged behind Node's in-memory socket state; API calls short-circuited on the stale DB read and returned 400. **Fixes**: (1) New `POST /api/internal/connect-event` (X-Internal-Secret guarded) — Node fires this on every `connection.open` (initial link + silent reconnect) → DB flips to `connected` instantly. (2) `_resolve_session` in `v2_compat.py` now runs `_live_check_and_maybe_reconnect()` against Node before rejecting — if Node says `connected`, DB is just stale and gets synced; if Node says `not_started` (process restart), backend eagerly calls `start_session` + polls up to 3s for reconnection. (3) Error messages now embed the LIVE Node status (not stale DB one). Also flipped default of API-key column on Sessions list to **unhidden** by default (user's own request — the other agent thought the key was hidden). |
| 16 | 8/8 (+70/70 reg) | **Live-check cache + API Reliability Dashboard** (both from user's list). (a) Live-check positive cache: `_resolve_session` caches "connected" results for 2s in-memory to avoid hammering Node during bulk-send workloads — negative states (disconnected/qr) bypass the cache. (b) New `metrics.py` module + `ApiMetricsMiddleware` records every `/api/v1/*` and `/api/v2/*` request into `db.api_metrics` with method/path/status/latency/session_id/user_id and a 7-day TTL index so the collection stays bounded. New admin endpoint `GET /api/admin/reliability?window=5m\|1h\|6h\|24h\|7d[&session_id=x][&bucket_minutes=1..60]` returns total/success_rate/4xx/5xx counts, p50/p95/p99 latency, top_failing_routes, by_status breakdown, per_session activity, and a time-series for charting. Plus `GET /api/admin/sessions` (flat list with customer email) for the filter dropdown. New page **`/app/reliability`** (admin-only, added to sidebar with `ChartLineUp` icon) shows the full dashboard: 4 KPI cards + 3 latency cards + stacked area chart (recharts, ok/4xx/5xx) + top failing routes panel + status code breakdown + per-session activity table. Auto-refresh every 15s (5m/1h) or 60s (longer windows). |
| 17 (2026-09-06) | 230/235 pytest (5 pre-existing env failures: webhook secrets unset, no git origin) + local QR/pair + GitHub-block simulation + UI smoke | **VPS pairing/QR failure RCA + migration**. Root causes: (1) OLD VPS could not reach WhatsApp at all (`408 WebSocket Error` ×20 → give up) — user migrated to a new VPS (Virtualizor clone, new IP, Cloudflare A record updated). (2) NEW VPS blocks `raw.githubusercontent.com` → Baileys `fetchLatestBaileysVersion()` failed → stale bundled WA-web version → `405 Connection Failure`. (3) Cloudflare replaces origin **502/504** with its own branded page, hiding the real Baileys error ("origin returned invalid/incomplete response"). **Fixes**: Baileys `7.0.0-rc9 → rc14` (WASM no-SIMD VPS fix, GHSA security fix, newer bundled version); `resolveWaVersion()` in `server.js` = GitHub (8s) → `fetchLatestWaWebVersion()` from web.whatsapp.com (8s) → in-memory cache → pinned `FALLBACK_WA_VERSION`; `dns.setDefaultResultOrder("ipv4first")`; `/pair` now waits (≤15s) for `sock.ws.isOpen && status==="qr"` before `requestPairingCode` and returns the disconnect label/code if the socket died; all wa-service proxy errors in `server.py` return **503 not 502**; new Node `GET /diag` + admin `GET /api/admin/wa-service/diag` (node/baileys versions, wa-web version+source, wasm_simd, reachability probes for github/whatsapp/whatsapp_sw, in-memory session list); ServiceCreate Step 5 shows a red **"WhatsApp connection failed"** banner with label/code/raw reason + **Retry connection** button (`POST /sessions/{id}/restart` then re-pair for phone method); polling also picks up `pairing_code` from status. Verified on the user's new VPS: diag shows baileys rc14, all probes ok, 2 sessions connected. |
| 18 (2026-09-06) | 76/76 pytest (iter4/5/9 + security) + 9 media tests + raw-contract curl (backend→Node) + **live on VPS: sendDocument(file) 201 + sendMessage(url) 201** | **Outbound media no longer touches disk**. Backend used to write every upload/URL-fetch to `/app/wa-service/uploads` and pass a `file_path` to Node (hardcoded pod path — also wrong on the VPS where the install dir is `/opt/wa9x`). Now `wa_client.send_media(session_id, to, data: bytes, caption, file_name, mime_type)` POSTs the raw bytes to Node `POST /sessions/:id/send-media` (`application/octet-stream`, metadata in `X-Wa-To`, `X-Wa-Mime`, `X-Wa-Caption-B64`, `X-Wa-File-Name-B64`; 180s timeout) and Node hands the **Buffer** straight to Baileys (`{image: buf}` / `{video: buf}` / `{audio: buf}` / `{document: buf}`). `express.raw` route-level, 120mb limit. `_send_media_one(..., data, ...)` signature changed; all 7 call sites (dashboard send-media, v1 media_url, scheduled dispatcher, v2 sendMessage url, sendDocument file/url, sendMessageFile, sendGroupFile) updated; `_read_upload_capped()` helper enforces `MAX_FILE_SIZE_BYTES` in memory. Node `send-group` with `url` also sends from Buffer (no tmp file). Inbound media still lands on local disk via Node (`wa-service/uploads/inbound`, persistent volume on VPS) — P2 backlog. Follow-ups from the live VPS debug: `url_guard.safe_get` now sends a browser-like UA/Accept (CDNs 403 the default python-httpx UA); `url_guard.describe_error()` keeps the exception class so `"Failed to fetch url: "` is never empty; `wa_client.send_media` translates old-Node `"to and file_path required"` into *"wa-service is running outdated code — restart wa9x-wa-service"*. ESLint 9 flat configs added (`/app/eslint.config.mjs` delegating to `frontend/eslint.config.js` + `wa-service/eslint.config.js`; 0 errors). |
...
| GET | `/api/admin/wa-service/diag` | Node/Baileys versions, WA-web version + source (github/wa-web/cache/fallback), wasm_simd, reachability probes, in-memory sessions — first thing to check when QR/pairing fails on a VPS |

## VPS ops notes (learned 2026-09-06)
- Production VPS: `/opt/wa9x`, supervisor programs `wa9x-backend` + `wa9x-wa-service`, logs `/var/log/wa9x-*.{out,err}.log`, `WA_AUTH_DIR=/var/lib/wa9x/auth`.
- **Never return 502/504 from FastAPI** — Cloudflare swaps them for its own error page and the JSON `detail` is lost. Use 503.
- If Node logs show `code=405 Connection Failure` → WA-web version stale (GitHub blocked). `code=408 WebSocket Error ()` repeatedly → VPS cannot reach WhatsApp at all (network/IP) — change host.
- After "Save to GitHub": `cd /opt/wa9x && git pull && (cd wa-service && yarn install) && (cd frontend && yarn build) && supervisorctl restart wa9x-backend wa9x-wa-service`. A stale Node process (no restart) shows up as `404 /diag`. **Backend and Node must be restarted together** — the send-media contract (raw body + `X-Wa-*` headers) changed in iter 18.
- Pre-existing pytest failures in the preview env (not bugs): 4× `TestBillingWebhooks` (webhook secrets unset → 503 by design) + `TestAdminSystemUpdate` (no git origin). Run tests with `export $(grep REACT_APP_BACKEND_URL /app/frontend/.env)`.
- The user's VPS provider blocks GitHub's IP range (185.199.x.x → raw.githubusercontent.com, GitHub Pages such as pdfobject.com). Never point tests or media URLs there; `picsum.photos` works from that VPS.
